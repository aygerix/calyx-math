...
| Sequences | Done, except that `Sort` returns the sorting permutation as a sequence of positions rather than a permutation group element (permutation groups come in Part V). |
...
carets, call frames, `eval` errors, syntax error recovery) and coercion
errors. Where the handbook's examples disagree with current Magma (they
were produced by many different versions), calyx follows current Magma:
...
- **Permutations.** `Sort` returns the sorting permutation as a sequence of
  positions rather than a permutation (permutation groups come in Part V).
...
`GF(q)`, one global `PolynomialRing(R)` per coefficient ring). The chapters'
intrinsics are still to be written, chapter by chapter, each checked
against Magma.

## Next

1. Part III chapter by chapter: integers, residue class rings, rationals,
   finite fields (including towers and embeddings), univariate and
   multivariate polynomials (factorisation via FLINT), real and complex
