...
Decomposition(Z, 6);
// A failed requirement names the intrinsic, unless the call is a statement.
x := Decomposition(Z, 6);
Decomposition(Integers(), 4); x := Decomposition(Integers(), 4);
Decomposition(Z, 0);
Decomposition(Z, 1);
Decomposition(Z, -1);
Decomposition(Z, 5/1);
...
x := RamificationIndex(I); x := RamificationIndex(P, 14); x := RamificationIndex(ideal< Z | 14 >, 7);
