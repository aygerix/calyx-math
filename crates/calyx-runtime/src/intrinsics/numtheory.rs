...
/// The primes up to `n` (a sieve of Eratosthenes).
...
fn primes_up_to_fn(_it: &mut Interp, a: &mut CallArgs) -> RResult<Vec<Value>> {
    ...
    let n = n.to_u64().filter(|&n| n <= 1 << 34).ok_or_else(|| RuntimeError::runtime("Limit is too large"))?;
    one(Value::int_seq(primes_up_to(n).into_iter().map(Integer::from_u64)))
}
...
fn nth_prime(_it: &mut Interp, a: &mut CallArgs) -> RResult<Vec<Value>> {
    ...
    let x = n as f64;
    let bound = if n < 6 { 15 } else { (x * (x.ln() + x.ln().ln())).ceil() as u64 + 1 };
    if bound <= 1 << 32 {
        return intv(Integer::from_u64(primes_up_to(bound)[n as usize - 1]));
    }
...
/// All square roots of `n` modulo `p^k` (at most a few thousand), the
/// one `sqrt_mod_prime_power` chooses first.
fn all_sqrt_mod_prime_power(n: &Integer, p: &Integer, k: u64) -> Vec<Integer> {
    let m = p.pow(k);
...
fn sqrts_mod(n: &Integer, m: &Integer, limit: usize) -> Vec<Integer> {
    ...
    for (p, k) in &f.factors {
        let pk = p.pow(*k);
        let roots = all_sqrt_mod_prime_power(n, p, *k);
